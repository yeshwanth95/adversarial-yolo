...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        contrast = torch.FloatTensor(batch_size).uniform_(self.min_contrast, self.max_contrast).to(device=DEVICE)
        contrast = contrast.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        contrast = contrast.expand(-1, -1, adv_batch.size(-3), adv_batch.size(-2), adv_batch.size(-1))
        contrast = contrast.to(device=DEVICE)


        # Create random brightness tensor
        brightness = torch.FloatTensor(batch_size).uniform_(self.min_brightness, self.max_brightness).to(device=DEVICE)
        brightness = brightness.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        brightness = brightness.expand(-1, -1, adv_batch.size(-3), adv_batch.size(-2), adv_batch.size(-1))
        brightness = brightness.to(device=DEVICE)


        # Create random noise tensor
        noise = torch.FloatTensor(adv_batch.size()).uniform_(-1, 1).to(device=DEVICE) * self.noise_factor
        ...
        msk_batch = torch.FloatTensor(cls_mask.size()).fill_(1).to(device=DEVICE) - cls_mask
        ...
        if do_rotate:
            angle = torch.FloatTensor(anglesize).uniform_(self.minangle, self.maxangle).to(device=DEVICE)
        else: 
            angle = torch.FloatTensor(anglesize).fill_(0).to(device=DEVICE)

        # Resizes and rotates
        current_patch_size = adv_patch.size(-1)
        lab_batch_scaled = torch.FloatTensor(lab_batch.size()).fill_(0).to(device=DEVICE)
        ...
        if(rand_loc):
            off_x = targetoff_x*(torch.FloatTensor(targetoff_x.size()).uniform_(-0.4,0.4).to(device=DEVICE))
            target_x = target_x + off_x
            off_y = targetoff_y*(torch.FloatTensor(targetoff_y.size()).uniform_(-0.4,0.4).to(device=DEVICE))
        ...
        theta = torch.FloatTensor(anglesize, 2, 3).fill_(0).to(device=DEVICE)
        theta[:, 0, 0] = cos/scale
        theta[:, 0, 1] = sin/scale
...
if __name__ == '__main__':
    ...
    darknet_model = darknet_model.to(device=DEVICE)
    patch_applier = PatchApplier().to(device=DEVICE)
    patch_transformer = PatchTransformer().to(device=DEVICE)
    prob_extractor = MaxProbExtractor(0, 80).to(device=DEVICE)
